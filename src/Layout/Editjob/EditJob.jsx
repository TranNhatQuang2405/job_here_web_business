...
import { jobBusiness, dropdownBusiness, companyBusiness } from "Business";
...
const Editjob = () => {
  ...
  const [jobInfo, setJobInfo] = useState({ industry: 0 });
  ...
  useEffect(() => {
    ...
    const first = async () => {
      ...
      if (!results.find((x) => x.data.httpCode !== 200)) {
        ...
        let jobData = results[4].data.objectData;
        let gender = results[6].data.objectData;
        let industry = results[7].data.objectData;
        let companyTmp = results[8].data.objectData;

        experience.forEach((element) => {
          element.checked = jobData?.experiences?.includes(element.experience);
        });
        jobType.forEach((element) => {
          element.checked = jobData?.jobTypes?.includes(element.jobType);
        });

        setListSkillSave(jobData?.jobSkills ?? []);
  ...
  const handleSubmit = async (e) => {
    ...
    master.experience.forEach((x) => {
      if (x.checked === true) experienceSave.push(x.experience);
    });
    let jobTypeSave = [];
    master.jobType.forEach((x) => {
      if (x.checked === true) jobTypeSave.push(x.jobType);
    });
    ...
    if (experienceSave.length === 0) {
      ...
      let result = await jobBusiness.UpdateJob(jobInfoCreate);
      if (result && result.data) {
        setAlert({
          title: t("business.manage.editJob.title"),
  ...
  const handleRemove = (skillId) => {
    let listSkillTmp = listSkillSave
      ? listSkillSave.filter((x) => x.skillId !== skillId)
      : [];
  ...
  return (
    <div>
      <PathTree lastPath={jobInfo.jobName || t(" ")} />
      {loading ? (
        <LoadingSpinner />
      ) : (
        <div className="mt-3">
          ...
          <div>
            <h3 className="text-center">{t("business.manage.editJob.title")}</h3>
          </div>
          <Form onSubmit={handleSubmit}>
              ...
              <Col className="mb-2">
                ...
                >
                  <option disabled value={""} hidden>
                    {t("business.manage.addJob.companyName")}
                  </option>
            ...
            <Row>
              <Col lg={8} className="mb-2">
                <Form.Label htmlFor="jobName">
                  {t("business.manage.addJob.jobName.label")}
                </Form.Label>
                <InputGroup>
                  <Form.Control
                    onChange={handleChange}
              ...
              <Col lg={4} className="mb-2">
                <Form.Label htmlFor="title">
                  {t("business.manage.addJob.titleInput.label")}
                </Form.Label>
                <Form.Select
                  onChange={handleChange}
                  id="title"
                ...
                >
                  <option disabled value={""} hidden>
                    {t("business.manage.addJob.titleInput")}
                  </option>
                  {master &&
                    master.title &&
                    master.title.map((x, index) => (
            ...
            <Row>
              <Col lg={8} className="mb-2">
                <Form.Label htmlFor="address">
                  {t("business.manage.addJob.address.label")}
                </Form.Label>
                <InputGroup>
                  <Form.Control
                    onChange={handleChange}
              ...
              <Col lg={4} className="mb-2">
                <Form.Label htmlFor="cityId">
                  {t("business.manage.addJob.city.label")}
                </Form.Label>
                <Form.Select onChange={handleChange} id="cityId" value={jobInfo.cityId}>
                  <option disabled value={""}>
                    {t("business.manage.addJob.city")}
                  </option>
                  {master &&
                    master.city &&
                    master.city.map((x, index) => (
                      <option key={index} value={x.cityId}>
                        {x.cityName}
                      </option>
                    ))}
                </Form.Select>
            ...
            <Row>
              <Col lg={8} className="mb-2">
                <Form.Label htmlFor="experiences">
                  {t("business.manage.addJob.experiences.label")}
                </Form.Label>
                <Row>
                  {master &&
                    master.experience &&
                    master.experience.map((x, k) => (
                      <Col lg={4} xs={6} key={k}>
                        <Form.Check
              ...
              <Col lg={4} className="mb-2">
                <Form.Label htmlFor="gender">
                  {t("business.manage.addJob.gender.label")}
                </Form.Label>
                <Form.Select
                  onChange={handleChange}
                  id="gender"
                ...
                >
                  <option disabled value={""}>
                    {t("business.manage.addJob.gender")}
                  </option>
                  {master &&
                    master.gender &&
                    master.gender.map((x, index) => (
                      <option key={index} value={x.gender}>
                        {x.genderName}
                      </option>
                    ))}
                </Form.Select>
            ...
            <Row>
              <Col lg={6} className="mb-2">
                <Form.Label htmlFor="industry">
                  {t("business.manage.addJob.industry.label")}
                </Form.Label>
                <Form.Select
                ...
                >
                  <option disabled value={0}>
                    {t("business.manage.addJob.industry")}
                  </option>
                  {master &&
                    master.industry &&
                    master.industry.map((x, index) => (
                      <option key={index} value={x.industryId}>
                        {x.industryName}
                      </option>
                    ))}
                </Form.Select>
              ...
              <Col lg={6} className="mb-2">
                <Form.Label htmlFor="skill">
                  {t("business.manage.addJob.skill.label")}
                </Form.Label>
                <Dropdown id="skill">
                  <Dropdown.Toggle className="w-100">
                    {loadSkill ? (
                      ...
                      t("business.manage.addJob.skill")
                    )}
                  </Dropdown.Toggle>
                  <Dropdown.Menu>
                    {listSkill &&
              ...
              <Col className="mb-2">
                <Form.Label htmlFor="listSkill">
                  {t("business.manage.addJob.listSkill.label")}
                </Form.Label>
                <div className="addJob__listSkill-bound">
                  {listSkillSave ? (
                    listSkillSave.map((x, key) => (
                      <div
                        key={key}
                        className="addJob__listSkill-item"
                        onClick={() => handleRemove(x.skillId)}
                      >
                        {x.skillName}
                      </div>
                    ))
            ...
            <Row>
              <Col lg={4} className="mb-2">
                <Form.Label htmlFor="companyName">
                  {t("business.manage.addJob.amount.label")}
                </Form.Label>
                <InputGroup>
                  <Form.Control
                    onChange={handleChange}
              ...
              <Col lg={8} className="mb-2">
                <Form.Label htmlFor="jobTypes">
                  {t("business.manage.addJob.jobTypes.label")}
                </Form.Label>
                <Row>
                  {master &&
                    master.jobType &&
                    master.jobType.map((x, k) => (
                      <Col lg={4} xs={6} key={k}>
                        <Form.Check
                          type="checkbox"
            ...
            <Row>
              <Col md={8} className="mb-2">
                <Form.Label htmlFor="salaryMin">
                  {t("business.manage.addJob.salary.label")}
                </Form.Label>
                <InputGroup>
                  <InputGroup.Text>
                    {t("business.manage.addJob.salaryMin.label")}
                  </InputGroup.Text>
                  <Form.Control
                    onChange={handleChange}
                    type="number"
                  ...
                  />
                  <InputGroup.Text>
                    {t("business.manage.addJob.salaryMax.label")}
                  </InputGroup.Text>
                  <Form.Control
                    onChange={handleChange}
                    type="number"
              ...
              <Col md={4} className="mb-2">
                <Form.Label htmlFor="unit">
                  {t("business.manage.addJob.unit.label")}
                </Form.Label>
                <Form.Select onChange={handleChange} id="unit" value={jobInfo.unit}>
                  <option disabled hidden value={""}>
                    {t("business.manage.addJob.unit")}
                  </option>
                  {master &&
                    master.unit &&
                    master.unit.map((x, index) => (
            ...
            <Row>
              <Col md={8} className="mb-2">
                <Form.Label htmlFor="salaryMin">
                  {t("business.manage.addJob.date.label")}
                </Form.Label>
                <InputGroup>
                  <InputGroup.Text>
                    {t("business.manage.addJob.startDate.label")}
                  </InputGroup.Text>
                  <Form.Control
                    onChange={handleChange}
                    type="date"
                  ...
                  />
                  <InputGroup.Text>
                    {t("business.manage.addJob.endDate.label")}
                  </InputGroup.Text>
                  <Form.Control
                    onChange={handleChange}
                    type="date"
              ...
              <Col xs={12} className="mb-2">
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html:
                      jobInfo.description || t("business.manage.addJob.description"),
                  }}
                ></div>
              </Col>
              <Col xs={12} className="mb-2">
                <Form.Label htmlFor="require">
            ...
            <div className="d-flex justify-content-end">
              <Button variant="danger" onClick={() => navigate("/manageJob")}>
