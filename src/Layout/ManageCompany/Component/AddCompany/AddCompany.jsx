...
function AddCompany({ show, onHide }) {
    ...
    return (
        <Modal className='modal__custom-bg' size="lg" fullscreen='lg-down' centered show={show} onHide={onHide}>
            <WarningModal ref={modalRef} />
            <AlertModal data={showAlert} onHide={showAlert.httpCode === 200 ? handleSuccess : handleFail} />
            <Modal.Header closeButton>
                <Modal.Title>{t("business.company.addCompanyTitle")}</Modal.Title>
            </Modal.Header>
            <Modal.Body>
                <Form className="AddCompany__body">
                    <Form.Label htmlFor="companyName">{t("business.company.add.companyName")}</Form.Label>
            ...
            <Modal.Footer>
                <Button variant="secondary" onClick={handleClose}>{t("business.company.add.close")}</Button>
                <Button variant="primary" disabled={loading} onClick={handSumit}>{loading ? <Spinner animation="border" size="sm" /> : t("business.company.add.submit")}</Button>
            </Modal.Footer>
        </Modal>
    )
}
